...
void Properties::computePktFlags(void)
{
    ...
    case PktType::CONNACK:
        ...
        allowedProperties +=
            pow2(PropertyId::SUBSCRIPTION_IDENTIFIER_AVAILABLE);
...
enum StatusCode Properties::append(ByteArray *key, ByteArray *value)
{
    ...
    this->_wireSize += propertyIdSize + binaryLenSize + key->size() +
...
enum StatusCode Properties::append(PropertyId id, const uint8_t *data,
                                   uint16_t size)
{
...
enum StatusCode Properties::append(PropertyId id, uint32_t value,
                                   uint32_t wireSize)
{
    if (!isAllowed(id)) {
        return StatusCode::INVALID_PROPERTY;
...
enum StatusCode Properties::assignedClientIdentifier(const uint8_t *data,
                                                     uint16_t size)
{
...
enum StatusCode Properties::authenticationMethod(const uint8_t *data,
                                                 uint16_t size)
{
...
enum StatusCode Properties::authenticationData(const uint8_t *data,
                                               uint16_t size)
{
...
enum StatusCode Properties::responseInformation(const uint8_t *data,
                                                uint16_t size)
{
...
{
    return userProperty((const uint8_t *)key, strlen(key), (const uint8_t *)val,
                        strlen(val));
}
...
static void writeNumProp(AppBuf &buf, uint32_t num, uint8_t size)
{
    ...
    WriteNumPtr ptrs[] = {&AppBuf::writeNum8, &AppBuf::writeNum16,
                          &AppBuf::writeNum32};
