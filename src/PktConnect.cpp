 ...
 */
static const uint32_t varHeaderSize =
    sizeof(protocolNameStr) + 1 + flagsSize + 2;
...
{
    Packet::minRemLen =
        varHeaderSize + propertyMinSize + stringLenSize + clientIdMinLen;
...
{
    if (topic == nullptr || topic_size < topicNameMinSize || msg == nullptr ||
        msg_size == 0) {
        return StatusCode::INVALID_ARGUMENT;
    }
...
enum StatusCode PktConnect::authenticationMethod(const uint8_t *data,
                                                 uint16_t size)
{
...
enum StatusCode PktConnect::authenticationData(const uint8_t *data,
                                               uint16_t size)
{
