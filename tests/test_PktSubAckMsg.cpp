...
template <typename T> int test(m5::ReasonCode *codes, std::size_t size)
{
...
int main(void)
{
    ...
    rc = test<m5::PktSubAck>(codes_suback,
                             sizeof(codes_suback) / sizeof(codes_suback[0]));
    test_rc(rc, "PktSubAck");

    rc = test<m5::PktUnsubAck>(codes_unsuback, sizeof(codes_unsuback) /
                                                   sizeof(codes_unsuback[0]));
