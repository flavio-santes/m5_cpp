...
static key_val KeyVal[] = {
    {.key = "Key #1", .val = "Val #1"}, {.key = "Key #2", .val = "Val #2"},
    {.key = "Key #3", .val = "Val #3"}, {.key = "Key #4", .val = "Val #4"},
    {.key = "Key #5", .val = "Val #5"}, {.key = nullptr, .val = nullptr}};
...
int test_Properties(void)
{
    ...
    while (KeyVal[i].key != nullptr) {
        ...
        wireSize += m5::propertyIdSize + m5::stringLenSize +
                    strlen(KeyVal[i].key) + m5::stringLenSize +
                    strlen(KeyVal[i].val);
        if (propList->wireSize() != wireSize) {
